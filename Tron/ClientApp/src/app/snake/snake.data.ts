export class SnakeData {
  private _widthSpriteNumber = 12;
  private _heightSpriteNumber = 8;
  private _isInitialized = false;
  private _snakeParts: SnakePart[];
  private _apple: SnakePart;
  private _score = 0;
  private _highestScore = 0;

  constructor(widthSpriteNumber, heightSpriteNumber) {
  ...
  initialize() {
    if (this._widthSpriteNumber > 1 && this._heightSpriteNumber > 1) {
      let intialY = (this._heightSpriteNumber % 2 == 0) ? this._heightSpriteNumber / 2 : this._heightSpriteNumber / 2 + 1;
  ...
  nextApple() {
    let emptyAreas: SnakePart[];
    emptyAreas = [];

    for (let i = 0; i < this._widthSpriteNumber; i++) {
      for (let j = 0; j < this._heightSpriteNumber; j++) {
        let snakePart = this.getSnakePart(i, j);
        if (!snakePart) {
    ...
    if (emptyAreas.length > 0) {
      let index = Math.floor(Math.random() * emptyAreas.length);
  ...
  moveSnake(action: ACTION) {
    if (this._snakeParts.length > 1) {
      // Get direction of snake
      let snakeDirection = (action != ACTION.NONE) ? SnakeData.getDirectionFromAction(action) : SnakeData.getDirectionFromStatus(this._snakeParts[0].Status);

      // Check if move is possible
      let checkResult = this.checkRedirection(snakeDirection);
      ...
      if (checkResult.isEating || this._snakeParts.length >= 3) {
        let snakePart = SnakePart.assign(this._snakeParts[0]);
  ...
  checkRedirection(snakeDirection: SnakeStatus): any {
  ...
  static getDirectionFromAction(action: ACTION): SnakeStatus {
  ...
  static getDirectionFromStatus(status: SnakeStatus): SnakeStatus {
  ...
  getSnakePart(x, y) {
    for (let i = 0; i < this._snakeParts.length; i++) {
  ...
  getWidthSpriteNumber() {
    this._widthSpriteNumber;
  }

  getHeightSpriteNumber() {
...
export class SnakePart {
  ...
  public X = 0;
  public Y = 0;
  public Status = SnakeStatus.None;
  ...
  static assign(snakePart: SnakePart): SnakePart {
